...
import { Document, WebIO, Material, Mesh, Primitive, Accessor } from '@gltf-transform/core';
import { EXTManifold, ManifoldPrimitive } from './manifold-gltf';
import { withModule } from './manifold-context';
import { ManifoldMesh } from '../types/manifold-mesh';
...
export async function manifoldToGLB(manifoldObject: any): Promise<Blob> {
  return withModule(async (module) => {
    ...
    gltfMesh.setExtension('EXT_mesh_manifold', manifoldPrimitive);
    ...
    const indices = document.createAccessor('primitive indices')
      .setBuffer(buffer)
    ...
    const positions = document.createAccessor('POSITION')
      .setBuffer(buffer)
    ...
    const normals = document.createAccessor('NORMAL')
      .setBuffer(buffer)
      .setType(Accessor.Type.VEC3)
    ...
    const node = document.createNode().setName('ManifoldNode').setMesh(gltfMesh);

    // Create a scene and add the node
    const scene = document.createScene().setName('ManifoldScene').addChild(node);
